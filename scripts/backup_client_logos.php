...
$pdo = get_db();
$rows = $pdo->query('SELECT id, name, logo_url FROM clients ORDER BY name')->fetchAll();
...
$fileTs = $backupDir . '/client-logos-' . $timestamp . '.json';
$fileLatest = $backupDir . '/client-logos-latest.json';
...
echo "Clients sauvegardés: " . count($rows) . PHP_EOL;
